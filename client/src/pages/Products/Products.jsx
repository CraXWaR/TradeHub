...
const Products = () => {
    ...
    return products.map((p) => {
      ...
      return (
        <div key={p.id || p._id || `${p.title}-${p.price}`} className="product-card bg-white/90 backdrop-blur-sm rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-orange-100 overflow-hidden flex flex-col">
          <div className="product-image relative w-full aspect-[4/3] bg-orange-50">
          ...
          <div className="p-4 flex flex-col gap-2">
            <h3 className="text-lg font-semibold text-gray-800 line-clamp-1 clamp-1">{p.title}</h3>
            <p className="text-sm text-gray-600 line-clamp-2 clamp-2 min-h-[2.5rem]">{p.description}</p>
  ...
  return (
    <div className="products-page min-h-screen bg-gradient-to-br from-orange-50 via-red-50 to-orange-100 text-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="products-header mb-8 text-center">
        ...
        {!loading && !error && products.length === 0 && (
          <div className="products-empty max-w-xl mx-auto bg-white/80 border border-orange-200 p-8 rounded-2xl text-center text-gray-700 shadow-sm">
        ...
        {!loading && !error && products.length > 0 && (
          <div className="products-grid grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {productCards}
          </div>
        )}
      </div>

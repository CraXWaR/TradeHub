...
export default function CartPage() {
    ...
    const {subtotal, shipping, tax, total, itemCount} = useMemo(() => {
        const s = items.reduce((acc, it) => acc + it.price * it.quantity, 0);
        ...
        return {
            subtotal: +s.toFixed(2), shipping: sh, tax: tx, total: tt, itemCount: c,
        };
    }, [items, applyGiftWrap]);
    ...
    return (<main className={style.main}>
        <div className={style.wrap}>
            ...
            {hasItems && (<section className={style.promo} aria-live="polite">
                <div className={style.promoRow}>
                    {subtotal >= FREE_SHIPPING_THRESHOLD ? (
                        <span className={style.promoOk}>🎉 Free shipping unlocked</span>) : (
                ...
                <div
                    ...
                    <span className={style.progressFill} style={{width: `${progress}%`}}/>
                </div>
            </section>)}

            {/* Grid layout */}
            {!hasItems ? (<EmptyCart/>) : (<div className={style.grid}>
                {/* Items */}
                        ...
                        <label className={style.gift} htmlFor="gift-wrap">
                            ...
                            <span className={style.giftBox}>
                                ...
                                <span className={style.giftText}>
                                  Add gift wrap <em className={style.giftPrice}>+ $4.00</em>
                                </span>
        ...
        {/* Mobile sticky bar */}
        {hasItems && (<div className={style.stickyPay} aria-label="Quick checkout bar">
